...
def load_train_data_for_rnn(cfg, x, y, aux, scaler, seq_len=None, last_end_time_idx=0):
    nt, nf, ngrid = x.shape
    mean, std = np.array(scaler[0]), np.array(scaler[1])

    # 如果提供了自定义序列长度，则使用它，否则使用配置中的序列长度
    seq_len = seq_len if seq_len is not None else cfg['seq_len']

    # 计算可用于训练的总批次数量
    total_batches = (nt - seq_len - cfg["forcast_time"])

    # 确保不会超出可用数据范围
    if last_end_time_idx + 1 > total_batches:
        last_end_time_idx = 0  # 重置为0，从头开始提取批次

    # 随机选择一个起始时间索引，确保不早于上一次的结束时间
    idx_time = np.random.randint(last_end_time_idx + 1, total_batches)

    # 随机选择网格点索引
    idx_grid = np.random.randint(0, ngrid, cfg['batch_size'])

    # 转置数据以匹配期望的形状
    x = np.transpose(x, (2, 0, 1))[idx_grid, idx_time:idx_time + seq_len]
    y = np.transpose(y, (1, 0))[idx_grid, idx_time + seq_len + cfg["forcast_time"]]
    aux = np.transpose(aux, (1, 0))[idx_grid]

    # 处理无穷大值和NaN值
    y[np.isinf(y)] = np.nan
    mask = ~np.isnan(y)  # 使用非NaN值作为掩码
    x = x[mask]
    y = y[mask]
    aux = aux[mask]
    x[np.isinf(x)] = np.nan
    ...
    # 更新上一次提取数据的结束时间索引
    next_last_end_time_idx = idx_time + seq_len + cfg["forcast_time"] - 1

    return x, y, aux, mean, std, last_end_time_idx
